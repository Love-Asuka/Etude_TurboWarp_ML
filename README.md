...
一个为[TurboWarp](https://turbowarp.org/)开发的机器学习扩展，允许用户在可视化编程环境中构建和训练神经网络模型。该扩展采用GPU加速，利用WebGL实现高效的矩阵运算，显著提升了模型训练和推理的性能。
...
- **GPU加速**: 利用WebGL技术实现GPU加速的矩阵运算，大幅提升计算性能
...
### 计算后端
为了提高计算性能，项目采用了基于WebGL的GPU加速计算后端，放弃了原有的纯CPU矩阵计算实现。通过利用GPU的并行计算能力，显著提升了矩阵运算的速度，特别是在处理大型神经网络时表现更为明显。

GPU加速后端的核心优势：
- 利用GPU的大规模并行处理能力，大幅提升矩阵运算性能
- 在处理大型矩阵时相比CPU实现有数量级的性能提升
- 保持与原有API的一致性，对上层代码透明

WebGL后端实现的关键技术点：
- 使用纹理（Texture）存储矩阵数据
- 通过片段着色器（Fragment Shader）实现矩阵运算
- 支持矩阵乘法和矩阵加法等核心运算
...
- 已完成激活函数独立成层的实现
- AdamW优化器支持更多自定义参数
- 引入基于WebGL的GPU加速计算后端，显著提升矩阵运算性能
